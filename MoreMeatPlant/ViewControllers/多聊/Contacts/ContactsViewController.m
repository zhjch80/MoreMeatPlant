...
- (UISearchBar *)searchBar
{
    if (_searchBar == nil) {
        ...
        _searchBar.placeholder = NSLocalizedString(@"search", @"搜索");
        ...
        [_searchController setDidSelectRowAtIndexPathCompletion:^(UITableView *tableView, NSIndexPath *indexPath) {
            ...
            if (loginUsername && loginUsername.length > 0) {
                if ([loginUsername isEqualToString:buddy.username]) {
                    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"prompt", @"提示") message:NSLocalizedString(@"friend.notChatSelf", @"你不能跟自己聊天哦！") delegate:nil cancelButtonTitle:NSLocalizedString(@"ok", @"好的") otherButtonTitles:nil, nil];
                    [alertView show];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    [tableView deselectRowAtIndexPath:indexPath animated:YES];
        ...
        if (loginUsername && loginUsername.length > 0) {
            if ([loginUsername isEqualToString:buddy.username]) {
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"prompt", @"提示") message:NSLocalizedString(@"friend.notChatSelf", @"您不能和自己聊天哦！") delegate:nil cancelButtonTitle:NSLocalizedString(@"ok", @"好的") otherButtonTitles:nil, nil];
                [alertView show];
...
- (void)cellImageViewLongPressAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:NSLocalizedString(@"cancel", @"取消") destructiveButtonTitle:NSLocalizedString(@"friend.block", @"加入黑名单") otherButtonTitles:nil, nil];
